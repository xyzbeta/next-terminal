...
func Auth(next echo.HandlerFunc) echo.HandlerFunc {
	...
	return func(c echo.Context) error {
		...
		// 从缓存获取用户可访问的 urlpath 模式列表，避免每次请求都重建
		var permissions []*urlpath.Path
		if cached, found := cache.PermissionCache.Get(account.ID); found {
			permissions = cached.([]*urlpath.Path)
		} else {
			var roles []string
			...
			// 扁平化：角色 → 菜单 → urlpath 模式列表，缓存后每次请求直接遍历匹配
			for _, role := range roles {
				menus := service.RoleService.GetMenuListByRole(role)
				for _, menu := range menus {
					permissions = append(permissions, service.MenuService.GetPermissionByMenu(menu)...)
				}
			}
			cache.PermissionCache.SetDefault(account.ID, permissions)
		}
